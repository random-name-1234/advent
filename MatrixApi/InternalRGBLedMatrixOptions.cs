...
internal struct InternalRGBLedMatrixOptions
{
    ...
    public InternalRGBLedMatrixOptions(RGBLedMatrixOptions opt)
    {
        ...
        rows = opt.Rows;
        cols = opt.Cols;
        hardware_mapping = StringToHGlobalAnsiOrZero(opt.HardwareMapping);
        inverse_colors = (byte)(opt.InverseColors ? 1 : 0);
        led_rgb_sequence = StringToHGlobalAnsiOrZero(opt.LedRgbSequence);
        pixel_mapper_config = StringToHGlobalAnsiOrZero(opt.PixelMapperConfig);
        panel_type = StringToHGlobalAnsiOrZero(opt.PanelType);
    ...
    private static IntPtr StringToHGlobalAnsiOrZero(string? value) =>
