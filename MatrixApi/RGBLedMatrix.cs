...
{
    private readonly IntPtr matrix;
    ...
    public RGBLedMatrix(RGBLedMatrixOptions options)
    {
        ...
        try
        {
            ...
            opt.hardware_mapping = options.HardwareMapping != null
                ? Marshal.StringToHGlobalAnsi(options.HardwareMapping)
                : IntPtr.Zero;
            opt.inverse_colors = (byte)(options.InverseColors ? 1 : 0);
            opt.led_rgb_sequence = options.LedRgbSequence != null
                ? Marshal.StringToHGlobalAnsi(options.LedRgbSequence)
                : IntPtr.Zero;
            opt.pixel_mapper_config = options.PixelMapperConfig != null
                ? Marshal.StringToHGlobalAnsi(options.PixelMapperConfig)
                : IntPtr.Zero;
            ...
            var cmdline_args = Environment.GetCommandLineArgs();
            ...
            // 0 as our sentinel).
            var slowdown_arg = new[]
                { cmdline_args[0], "--led-slowdown-gpio=" + (options.GpioSlowdown == 0 ? 1 : options.GpioSlowdown) };

            var argv = new string[2 + cmdline_args.Length - 1];
...
public struct RGBLedMatrixOptions
{
    ...
    /// <summary>
    ///     In case the internal sequence of mapping is not "RGB", this contains the real mapping. Some panels mix up these
    ///     colors.
    /// </summary>
    ...
    /// <summary>
    ///     Allow to use the hardware subsystem to create pulses. This won't do anything if output enable is not connected to
    ///     GPIO 18.
    /// </summary>
